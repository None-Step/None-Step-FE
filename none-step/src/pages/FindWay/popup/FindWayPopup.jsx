...
const getLineColor = (lineName, region) => {
  ...
  switch (region) {
    case '수도권':
      ...
      if (lineName.includes('경의중앙선'))
        return theme.capitalLines.gyeonguiJungang;
      if (lineName.includes('경춘선')) return theme.capitalLines.gyeongchun;
      if (lineName.includes('수인분당선'))
        return theme.capitalLines.suinBundang;
      ...
      if (lineName.includes('우이.신설선'))
        return theme.capitalLines.wooyiShinseol;
      if (lineName.includes('김포골드라인'))
        return theme.capitalLines.gimpoGold;
...
const FindWayPopup = ({
  ...
  const fetchStationRegion = useCallback(async (latitude, longitude) => {
    try {
      const response = await axiosInstance.get(
        `/nonestep/subway/now-station?latitude=${latitude}&longitude=${longitude}`
      );
    ...
    for (let i = 0; i < routeInfo.subwayRoute.stationSet.stations.length; i++) {
      ...
      if (nextStation) {
        const nextLine = routeInfo.subwayRoute.driveInfoSet.driveInfo.find(
          info =>
            getMappedStationName(info.startName) ===
            getMappedStationName(nextStation.startName)
        );
      ...
      if (currentLineName !== currentLine.laneName) {
        ...
        try {
          ...
          const stationRegion = await fetchStationRegion(
          ...
          console.error(
            `Error fetching color for station ${station.startName}:`,
            error
          );
  ...
  return (
    <PopupContainer>
      ...
      <TabContainer>
        <Tab
          $active={activeTab === '도보 및 자전거'}
          onClick={() => setActiveTab('도보 및 자전거')}
        >
          도보 및 자전거
        </Tab>
        <Tab
          $active={activeTab === '지하철 경로'}
          onClick={() => setActiveTab('지하철 경로')}
        >
          지하철 경로
        </Tab>
      </TabContainer>
      ...
      <TabContent $active={activeTab === '도보 및 자전거'}>
        {routeInfo.isOverDistance ? (
          <RouteDetail>
            {'반경 20km 이상의 거리는 노출되지 않으니 미 노출 시 가까운 역으로 재검색 부탁 드립니다.' ||
              '도보 경로를 제공할 수 없습니다.'}
          </RouteDetail>
        ) : (
          <>
            <RouteOption>
              <RouteInfo>
                ...
                {routeInfo.walkTime === '0분' ? (
                  <RouteDetail>
                    반경 20km 이상의 거리는 노출되지 않으니 미 노출 시 가까운
                    역으로 재검색 부탁 드립니다.
                  </RouteDetail>
                ) : (
          ...
          <SubwayRouteInfo>
            <RouteOverview>
              <OverviewItem>
                ...
                <OverviewValue>
                  {formatTime(routeInfo.subwayRoute.globalTravelTime)}
                </OverviewValue>
              </OverviewItem>
              <OverviewItem>
                <OverviewLabel>환승 횟수</OverviewLabel>
                <OverviewValue>
                  {routeInfo.subwayRoute.exChangeInfoSet?.exChangeInfo
                    ?.length || 0}
                </OverviewValue>
              </OverviewItem>
              <OverviewItem>
              ...
              <OverviewItem>
                ...
                <OverviewValue>
                  {routeInfo.subwayRoute.globalDistance?.toFixed(1)}km
                </OverviewValue>
              </OverviewItem>
            </RouteOverview>
            ...
            <StationList>
              {coloredStations.map((station, index) => {
                const isTransfer =
                  routeInfo.subwayRoute.exChangeInfoSet?.exChangeInfo?.find(
                    transfer =>
                      getMappedStationName(transfer.exName) ===
                      getMappedStationName(station.endName)
                  );
                ...
                return (
                  <React.Fragment key={index}>
                    ...
                    {isTransfer && (
                      <StationItem>
                        <StationDot
                          color={nextStation?.color || theme.colors.gray03}
                        />
                        <StationInfo>
                          ...
                          <TransferInfo>
                            ...
                            {isTransfer.exWalkTime && (
                              <>
                                , 도보{' '}
                                {formatTransferTime(isTransfer.exWalkTime)}
                              </>
                            )}
                          </TransferInfo>
                        </StationInfo>
