...
const MyPage = () => {
  ...
  const handlePasswordChange = useCallback(() => {
    ...
    axiosInstance.put('/nonestep/member/modify-pass', passwordData)
      .then(response => {
  ...
  const handleEmailChange = useCallback(() => {
    axiosInstance.put('/nonestep/member/modify-mail', { memberMail: newEmail })
      .then(response => {
        if (response.data.message.toLowerCase() === 'success') {
  ...
  // 인증번호 발송하기
  const sendVerificationCode = useCallback((event) => {
    event.preventDefault();
    if(currentPhone) {
      axiosInstance.post(`/nonestep/member/phone`, { memberPhone: currentPhone })
        .then((response) => {
  ...
  const handlePhoneChange = useCallback(() => {
    if (verificationPassed) {
      axiosInstance.put('/nonestep/member/modify-phone', { memberPhone: newPhone })
        .then(response => {
          if (response.data.message.toLowerCase() === 'success') {
...
{editModalOpen && (
  ...
  <ModalContainer>
      ...
      <>
        <InputWrap>
          <InputForm
            ...
            value={currentPhone}
            onChange={(value) => setCurrentPhone(value)}
            onValidationChange={(isValid) => setPhoneNumberValid(isValid)}
          />
          <SubmitBut onClick={sendVerificationCode} disabled={!phoneNumberValid}>
    ...
    <Button 
      onClick={
        editType === '비밀번호' ? handlePasswordChange :
        editType === '이메일' ? handleEmailChange :
        handlePhoneChange
      } 
      submitMessage="저장하기" 
    />
...
{ isWithdrawCompleteOpen &&
  <WithdrawContainer>
    ...
    <ButWrap>
      <SubmitButton onClick={handleWithdrawComplete}>확인</SubmitButton>
    </ButWrap>
  </WithdrawContainer>
}
